...
template <typename T> bool Array<T>::validateIndex(long long index) const {
    return index >= base_ &&
           index < base_ + static_cast<long long>(this->size());
}
...
bool CompactMatrix<T>::validateIndices(long long index1,
    ...
    return index1 >= dimension1_.getBase() &&
           index1 < dimension1_.getBase() +
                        static_cast<long long>(dimension1_.getSize()) &&
           index2 >= dimension2_.getBase() &&
           index2 < dimension2_.getBase() +
                        static_cast<long long>(dimension2_.getSize());
}
